import { users, type User, type InsertUser, transactions, type Transaction, type InsertTransaction, gameHistory, type GameHistory, type InsertGameHistory, kycDocuments, type KycDocument, type InsertKycDocument, sportsEvents, type SportsEvent, type InsertSportsEvent, sportsBets, type SportsBet, type InsertSportsBet, favorites, type Favorite, type InsertFavorite, slotGames, type SlotGame, type InsertSlotGame, slotSessions, type SlotSession, type InsertSlotSession, type CrashGame, type CrashBet, userActivities, type UserActivity, type InsertUserActivity } from "@shared/schema";
...
export interface IStorage {
  ...
  updateProfileImage(id: number, imageUrl: string): Promise<User | undefined>;
...
export class MemStorage implements IStorage {
  ...
  async getAllUserActivities(limit?: number): Promise<UserActivity[]> {
    let activities = Array.from(this.userActivities.values())
      .sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
  ...
  async updateProfileImage(id: number, imageUrl: string): Promise<User | undefined> {
    const user = await this.getUser(id);
    if (!user) return undefined;
